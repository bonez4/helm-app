...
- **Per-day Routes editor (David only)** — *replaces* the single Route + Route Note section for David. Renders one row per currently-toggled-on pickup day (re-renders live when a day is toggled), each with a Day label, Route dropdown (no route / R1-R14), and a route-note text input. Pre-populates from existing `route_assignments` rows. On save: upserts a row per selected day, deletes rows for days that were toggled off. Also syncs `clients.route` and `clients.route_note` to the first selected day's values so non-David users still see a sensible single-route fallback on the lookup card. Other users still see the existing single-route UI unchanged.
...
- **May 7, 2026** — **Editable Account # + per-day pair auto-fill + Add-Client numeric guard.** Three small but useful pickups. (1) Account # in the Edit Client form is now editable. Changing it triggers a true PK rename via `migrateAccountId(oldId, newId)`: insert new client row (copy all fields), `UPDATE` every `notes` / `skips` / `route_assignments` row from old → new, then delete old. Confirm dialog before any writes; on sub-step failure, attempts to roll back the new row. Lets the office fix a client whose acct # got accidentally saved as their last name. (2) Per-day editor: setting route or route-note on one day of a standard pickup pair (Mon↔Thu, Tue↔Fri, Wed↔Sat) auto-copies the value to the partner when the partner is also selected and its same field is empty. Manual values are never overwritten. So 2x/wk Wed+Sat clients fill Sat automatically; 3x/wk Mon+Wed+Sat clients pair Wed↔Sat but leave Monday standalone (its pair Thursday isn't selected). (3) Add Client guards against the "tabbed past Account # and typed name into the wrong field" failure mode: input has `autocomplete=off`, `inputmode=numeric`, `pattern=[0-9]*`, and `addClient()` rejects any non-digit value with a clear toast.
- **May 7, 2026** — **Per-day route resolution in Daily Action / Notes Added / Everything reports + per-day editor in Edit Client (David only).** Reports now look up each note's route via `route_assignments` keyed on `(client_id, day_of_week_of_action_date)`, falling back to `clients.route` only if no per-day override exists. Same per-day resolution for the inline `📋 route_note` line. So a multi-day client like acct 208849 (Mon=R4 / Wed=R2 / Sat=R2) lands its notes on the right route automatically. Implementation: two helpers `resolveRoute()` and `resolveRouteNote()` read from a `resiRoutesByKey` Map populated by `rrLoadData()`. The three report functions `await rrLoadData()` before rendering. — Edit Client form: for David, a new **Per-day Routes** section replaces the single Route + Route Note; renders one row per toggled-on pickup day (live re-renders on toggle), each with Day / Route dropdown / Route note input. Pre-populated from existing rows. On save: upserts route_assignments per selected day, deletes for toggled-off days, syncs `clients.route` and `clients.route_note` to the first day's values for backward-compat fallback. Cache invalidated after writes.
- **May 7, 2026** — **3X WK note action added.** New action type slotted between `2X WK` and `LPU` in `ACTION_TYPES`, with its own `at-3xwk` indigo pill (#a5b4fc bg / #312e81 text). Auto-flows through every consumer — note dropdown, action chip rendering, action filters in the Reports/Notes Added/Everything reports — because they all read from `ACTION_TYPES`. Use case: clients with Mon/Wed/Sat or other tri-weekly schedules. The `clients.service_day` field is already comma-separated and the day-toggle UI has no cap, so 3-day pickups have actually worked since launch — this just gives the office a way to *log* a 3X WK reactivation call. The new `route_assignments` table also already supports per-day routes for these clients (e.g. acct 208849 has Mon=R4 / Wed=R2 / Sat=R2 from import; 26 clients have 3+ day-rows, one has all 6).
