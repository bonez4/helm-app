...
- **Office-software integration kickoff (for the IT guy).** Added **`INTEGRATION.md`** (dev guide: shared-Supabase-as-hub, connection surfaces, RLS/security model, data-model map with join keys, conventions + gotchas) and **`WISHLIST.md`** (reporting wants + the two "big projects," each tagged *in-app now* / *needs a tracker* / *needs external integration*). Commits `c01bad3` / `962cd62`. *(`INTEGRATION.md` was later deleted from the working tree — recover via `git checkout INTEGRATION.md` if wanted.)*
