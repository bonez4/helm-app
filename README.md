...
- **David only**: the **Supervisor Resolutions** review inbox — a green **"R"** topbar badge counting supervisor-resolved complaints David hasn't reviewed yet, opening a list of each with the resolver's **resolution note front-and-center** + a per-item "Mark reviewed". Review-state is per-David in `localStorage` (no DB). See the Supervisor Queue section.
...
- **Company scope toggle** — a **REIS · BOTH · SANTOS** segmented control in the topbar (all users) filters the client search to one company (`2xxxxx` / `3xxxxx`) or both. Choice persists per-browser (`localStorage` `helm_co_filter`); the search hint reflects the active scope. Color-coded (REIS green, SANTOS blue, BOTH teal).
...
### Edit Client
- Name / address / phone / email / pickup days / **company-aware route dropdown** / autopay
- Routes chosen from "Select Route" dropdown; renders as a navy pill on the card
- **Route dropdown max is company-scoped** (`routeCountForCompany`): **Santos = R1–R19**, **Reis = R1–R14**. Applies in both the Add Client and Edit Client per-day route pickers (Add keys off the selected company toggle; Edit off the client's account #).
...
- **Rep self-edit:** on a client's profile, the person who logged a complaint sees an **Edit** button on their own complaint row while it's still open — `status='new'` **or** `case_open` (`complaintEditableByRep` = `logged_by`-matches-me AND not resolved/ignored). It reopens the same modal in edit mode (type / notes / driver / callback), does an UPDATE (not a new row), and writes an `edited` action to the audit log. Supervisor auto-routing is only recomputed on a type change when the complaint is still `new` (an opened case is never re-routed out from under a supervisor). Once it's **resolved or ignored** the Edit button disappears — it locks so a closed case's record can't be rewritten.
...
- **🚛 Drivers** — a per-driver leaderboard (every complaint that has a `driver_name`) over a date range → drill into a driver's complaints. `driver_name` is the **"Driver involved" entered at log time** (the driver the complaint is *about*). The supervisor **"Spoke to Driver" step names the fixer and is NOT counted** here (it stays in the action log) — earlier it was back-filled into `driver_name`, which wrongly counted fixers; removed, plus a one-time SQL cleanup of historical rows. Driver names are grouped **case/whitespace-insensitively** (`groupByDriver` / `driverCanonKey`) so "MARLON" and "Marlon" merge into one row (shown as the most-used spelling).
- **📈 Insights** — a dynamic analytics dashboard: KPI strip (incl. **repeat clients** + % from repeat); **Week-over-week + Month-over-month count tables** (`ciWoWMoM` — contiguous weeks/months incl. zero-count periods, last 12 of each, with a ▲/▼ change-vs-prior-period column; a rolling view independent of the range filter); **per-route + per-driver trend leaderboards** (most complaints → least, each with a **▲ rising / ▼ falling** arrow comparing the second half of the range to the first, click a row to drill); live **Chart.js** charts (complaints over time / by-type doughnut / by-route bar / by-driver bar — click a bar to drill); and a **repeat-customer table** (2+ complaints, click to drill). Everything clicks through to the case. **Route is resolved from the client's CURRENT route** (`complaintRouteLabelLive` → live `clients` lookup, not the log-time snapshot), so complaints reflect the latest routing (e.g. after the Santos route import). Route buckets are **company-scoped** — the label is prefixed with the company (`REIS · t/f-08` vs `SANTOS · t/f-08`) since Reis Route 8 ≠ Santos Route 8, so the two companies never share a by-route row.
...
- **The call is its own task — separate from resolving the complaint** (the call is usually made *after* the complaint is closed). It stays outstanding until someone hits **✓ Mark call made** (`markCallbackDone` → sets `callback_done`/`callback_done_at`/`callback_done_by`, logs a `callback_made` action). Resolving the complaint does **not** make the call.
- **Tracked** via the **☎ CALLBACK** card chip (blue = owed, green ✓ = made), the case-detail banner, and a **☎ Needs Callback** stat card in the console (`callbackOutstanding` = `needs_callback && !callback_done`, counted **regardless of complaint status** so a resolved-but-uncalled client still shows). Test client `999999` excluded from the worklist.
- **Notified on resolve:** if a resolved complaint **still owes a call**, a `callback_resolved` broadcast pings **everyone with console access** (David, Chris, Esme, Jackie, Kobie) — a toast + browser notification "call still owed" — so the closed complaint doesn't bury the outstanding call. The resolver is skipped. Needs HELM open in a tab.
...
- **Solve screen (stage, then commit):** ordered **toggle** buttons **Spoke to Driver** (prompts for the driver's name) · **Spoke to Client** · **Contacted Office** · **On-site Visit** — tap to highlight (tap again to undo); add a note; then **✓ Resolve** writes the staged actions + note to `complaint_actions` all at once (the note becomes the resolution, else an auto-summary of the actions). **↩ Send back to David** (escalate; also commits staged actions) + **Reopen**. Header has click-to-call + a ⚠ repeat-client flag + a **☎ CALLBACK** chip. Staged selections are transient until Resolve/Send-back. *(Changed 2026-06-17 from auto-log-on-tap.)*
- **Callback-aware confirm popup** (`openSupCallbackModal`): if the case still owes a follow-up call, **Resolve** and **Send back to David** first pop a modal surfacing the **callback number** (the one the client gave, else the number on file — click-to-call), the **callback note**, and the **original complaint**, so nothing is lost in the handoff. Send-back replaced the old `prompt()` with this modal (reason box included); the escalate audit note also records that a call is still owed + the number.
...
- **Supervisor Resolutions review inbox (David only) — so resolution notes don't get lost mid-week.** A green **"R"** topbar badge shows the count of supervisor-resolved complaints David hasn't reviewed; it polls every 60s, refreshes on focus, and bumps in realtime when a supervisor resolves. Clicking it opens a review modal (`openSupResReview`) — **To review / Reviewed / All** tabs — where each row shows client · type · route · driver/callback flags, **who resolved it + when**, the original complaint, and the **resolution note in a prominent green box**, with **✓ Mark reviewed** (checks it off, drops the badge) and **Open full case →** (jumps into the console). "Supervisor-resolved" = resolved by an `is_supervisor` user (fallback: routed + not David/Chris); test client `999999` excluded. Reviewed-state is per-David in `localStorage` (`helm_sup_res_reviewed`) — no DB migration, but per-device.
...
-- The follow-up call is its own task, tracked separately from complaint resolution:
ALTER TABLE complaints ADD COLUMN IF NOT EXISTS callback_done    BOOLEAN NOT NULL DEFAULT false;
ALTER TABLE complaints ADD COLUMN IF NOT EXISTS callback_done_at TIMESTAMPTZ;
ALTER TABLE complaints ADD COLUMN IF NOT EXISTS callback_done_by TEXT;
...
- **Supervisor role + queue.** New `users.is_supervisor` flag (**Dane + Brian**). A big green **"Complaint Queue"** topbar pill (supervisors land on it at login; David/admin get a small oversight icon). Mobile-first **My / Available / Solved** tabs; **claim** from the pool; a fast **Solve** screen — ordered **toggle** buttons **Spoke to Driver** (prompts for the driver's name) · **Spoke to Client** · **Contacted Office** · **On-site Visit** (tap to highlight/stage — not logged yet) + an always-on **notes** field; **✓ Resolve** writes the staged actions + note to `complaint_actions` at once (**↩ Send back to David** / **Reopen** also commit); click-to-call + a ⚠ repeat-client flag. New `complaints` columns `routed_to_supervisor` / `assigned_to` / `assigned_at` / `assigned_by` (user-applied SQL). Commits `38c8ef7` → `c480ebd` → `1a22f5e`.
...
- **HELM scale report — Reis pile (ZZDELTA) now included in Total Inbound.** The daily email + Intercompany Rolloff recomputed Total Inbound as rolloff + walk-in, omitting `pile_tons`, so MTD ran ~54 t low vs the stored `total_inbound_tons`. Fixed by folding `pile_tons` into the External (walk-in) bucket in `irrLoadYTD.sum()` — corrects every period (Today / MTD / YTD / YoY) in both views. Commit `8c9e871`. *(Reis **rolloff** itself is computed correctly — keyed on the rolloff service line item; 6/4 = 17.61 t. The "Internal Volume" number blends Reis + Island + East End, so rolloff isn't visible on its own — a per-line breakout on the email is a possible follow-up.)*
- **Scale-parser audit — two KNOWN OPEN issues (not yet fixed):**
  1. **Tonnage undercount on non-"transfer fee" lines.** The parser only sums weight from `REIS SITE TRANSFER FEE` lines, missing tonnage billed on other `$/tn` lines — e.g. `METAL DROPPED OFF @ REIS YARD` (East End read 6.02 t vs the true **6.84 t** on 6/4; this ~0.8 t/day gap is why the Delta-side `deltawaste_display` report runs higher than HELM). Fix = count Net from any `$ / tn` line, not just transfer fees. **Offered, awaiting go-ahead.**
  2. **`kpiAggregate` still excludes pile.** The Scale KPIs grid + Scale Monthly Review use a separate aggregator that doesn't fold pile in (has walk-in avg-tons/ticket implications). Not changed yet.
- **Office-software integration kickoff (for the IT guy).** Added **`WISHLIST.md`** (reporting wants + the two "big projects," each tagged *in-app now* / *needs a tracker* / *needs external integration*), plus originally `INTEGRATION.md` (a shared-Supabase-as-hub dev guide). Commits `c01bad3` / `962cd62`. *(`INTEGRATION.md` was **removed 2026-06** — its Supabase-as-hub premise was superseded once the IT guy moved to scraping Delta into his own Metabase; the current IT-guy deliverable is the standalone pricing-model spec, kept outside the repo. Recover from git history if ever needed.)*
- **`client_status_log` table + trigger** (user-applied SQL) now captures every `clients.status` change (insert/update) going forward — the foundation for clientele reporting (turn-ons / turn-offs / pauses / new creations). The report on top of it is still to build.
...
- **BEACON dispatch refinements.** A Notes field on every dispatch stage (`dispatch_jobs.notes`); the Dispatch panel shows the full contact block (name · acct # · address · click-to-call phone); and dispatch jobs overlay **live** `clients` data (name/address/phone) on every load, so HELM edits always propagate to the board/modal/ticket even mid-pipeline (verified via a multi-agent audit that also caught a stale-history reload gap). Commits `d946604`→`bff4e4d`.
- **BEACON dispatch: confirm-charges gate.** "Mark complete" now opens a Confirm Charges modal (line items pre-loaded from Outreach, editable) before a job lands in Completion, so billing is a deliberate step. Commit `b1bfd0b`.
- **BEACON Activity Report.** Date-filtered report (`#activity`, from the Reports tab) of images / videos / notes added to account History (`commercial_history`) — From/To + presets, summary totals + per-account table + Print. No new table. Commit `b1889f1`.
